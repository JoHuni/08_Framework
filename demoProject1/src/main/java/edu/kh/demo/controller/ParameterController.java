...
public class ParameterController {
	...
	/* 1. HTTPServletRequest.getPrameter("key") 이용 */
	
	// HTTPServletRequest :
	//  - 요청 클라이언트 정보, 제출된 파라미터 등을 저장하고 있는 객체
	...
	public String paramTest(HttpServletRequest req) {
		...
		// -> 값이 잘못된 경우 -> 값 추적
		log.debug("inputName : " + name);
		log.debug("inputAge : " + age);
		log.debug("inputAddress : " + address);
		
		/* Spring에서 Redirect(재요청) 하는 방법 
		 * 
		 * - Controller 메서드 반환 값 앞에
		 * 	 "redirect:요청주소"; 작성
		 * */
	...
	/* 3. @RequestParam 어노테이션 - 낱개(한 개, 단 수)개 파라미터 얻어오기 */
	// String[]
	// List<자료형>
	// Map<String, Object>
	...
	public String paramTest3(
			...
			@RequestParam Map<String/*여기 무조건 String*/, Object> paramMap
			) {
		...
		// -> 문제점 : key값(name 속성 값)이 중복되면 덮어쓰기가 된다!
