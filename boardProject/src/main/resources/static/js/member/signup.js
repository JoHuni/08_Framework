...
memberNickname.addEventListener("input", e => {
    ...
    if(!regExp.test(inputNickname)){
        nickMessage.innerText = "형식에 맞는 닉네임을 입력해주세요.";
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    checkObj.authKey = false; // 인증 유혀성 검사 여부 False
    ...
    authTimer = setInterval(() => {
        authKeyMessage.innerHTML = `${addZero(min)}:${addZero(sec)}`;
...
// 전달 받은 숫자가 10 미만인 경우(한 자리) 앞에 0붙여서 반환
